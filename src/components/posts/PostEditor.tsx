...
const PostEditor = forwardRef<PostEditorHandle, PostEditorProps>(function PostEditor({
  ...
  const contentMediaPlaceholderRef = useRef<{ start: number; end: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    if (mode !== "create" || copiedFromPostId || hasRestoredDraftRef.current) {
      return;
    }
    ...
    const applyDraftToEditor = (draft: PostDraftPayload) => {
      ...
      setTags(filterVisibleCategoryNames(Array.isArray(draft.tags) ? draft.tags : []));
  ...
  function insertContentMediaPlaceholder() {
    ...
    contentMediaPlaceholderRef.current = { start: insertStart, end: insertEnd };
    setContent(nextContent);
  ...
  async function insertContentMediaAtCursor(files: File[]) {
    ...
    try {
      ...
      for (const file of mediaFiles) {
        ...
        markdownBlocks.push(buildContentMediaMarkdown(file, publicUrl));
      }
  ...
  function handleThumbnailDragOver(event: React.DragEvent<HTMLElement>) {
    if (
      isBusy ||
      !hasThumbnailImageInDataTransfer(event.dataTransfer)
    ) {
      return;
    }
  ...
  async function handleThumbnailCropComplete(result: ImageCropCompleteResult) {
  ...
  async function handleThumbnailCropDelete() {
    ...
    try {
      ...
      if (!isContentMedia && sourceKey?.startsWith("http")) {
        const path = getStorageObjectPathFromPublicUrl(sourceKey);
    ...
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          isBanner,
          isSecret,
          ...(trimmedSecretPassword ? { secretPassword: trimmedSecretPassword } : {}),
          ...(mode === "edit" ? { isHidden } : {}),
    ...
    >
      ...
      {categoryLocked ? null : (
        <input
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter") {
              if (isTagComposingRef.current || event.nativeEvent.isComposing) {
                return;
              }
    ...
    <section className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {mode === "create" ? createSecretToggleButton : inlineSecretToggleButton}
          {mode === "create" ? (
            <Button
              type="button"
              variant="outline"
