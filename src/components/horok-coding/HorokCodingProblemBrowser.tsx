...
type HorokCodingProblemBrowserProps = {
...
function parseCreatedAt(problem: HorokCodingProblem) {
...
export default function HorokCodingProblemBrowser({
  problems,
  selectedLevel: selectedLevelProp,
  onSelectedLevelChange,
  showLevelTabs = true,
  emptyMessage = "준비중입니다.",
}: HorokCodingProblemBrowserProps) {
  const [sortValue, setSortValue] = useState<SortValue>("number-asc");
  const problemsByLevel = Object.entries(
    problems.reduce<Record<string, HorokCodingProblem[]>>((groups, problem) => {
          ...
          {paginatedProblems.length > 0 ? (
            <div className="grid gap-4 md:grid-cols-2">
              {paginatedProblems.map((problem) => (
                <Link
                  key={problem.number}
                  href={`/horok-coding/${problem.number}`}
