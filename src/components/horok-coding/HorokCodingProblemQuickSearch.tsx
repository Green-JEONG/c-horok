...
type HorokCodingProblemQuickSearchProps = {
  number?: number;
  title?: string;
...
const HOROK_CODING_SEARCH_MIN_WIDTH = 220;

export default function HorokCodingProblemQuickSearch({
  number,
  title,
  problems,
  alwaysExpanded = false,
}: HorokCodingProblemQuickSearchProps) {
  ...
  const fetchBookmarkStatus = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/horok-coding/saved-code?problemSlug=${encodeURIComponent(
          currentProblem.slug,
        )}`,
      );
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("coding-bookmark-updated", handleUpdate);
    return () => {
      window.removeEventListener("coding-bookmark-updated", handleUpdate);
    };
  }, [fetchBookmarkStatus]);
  ...
  const handleBookmarkToggle = async (e: React.MouseEvent) => {
    ...
    try {
      if (isBookmarked) {
        ...
        if (response.ok) {
          setIsBookmarked(false);
          window.dispatchEvent(new Event("coding-bookmark-updated"));
          router.refresh();
        } else {
          alert("북마크 해제에 실패했습니다.");
        }
      } else {
        const response = await fetch("/api/horok-coding/saved-code", {
          method: "POST",
          headers: {
  ...
  const handleSelectProblem = (slug: string) => {
    ...
    setIsEditing(false);
    setQuery("");
    router.push(`/horok-coding/${selectedProblem.number}`);
  };
