...
function getTestCaseConditions(
  testCase: HorokCoteProblem["testCases"][number],
) {
...
export default function HorokCoteIDE({
  problem,
  onSolved,
  elapsedSeconds = 0,
}: HorokCoteIDEProps) {
  ...
  useEffect(() => {
    ...
    const loadSavedCodes = async () => {
      try {
        const response = await fetch(
          `/api/horok-cote/saved-code?problemSlug=${encodeURIComponent(problem.slug)}`,
        );
        if (response.ok) {
          const data = await response.json();
          if (data) {
  ...
  async function handleRun() {
    ...
    void fetch("/api/horok-cote/saved-code", {
      method: "POST",
      headers: {
    ...
    void fetch("/api/horok-cote/submissions", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
