...
export async function POST(req: Request) {
  ...
  const codingProblemProgressDelegate = (
    prisma as typeof prisma & { codingProblemProgress: unknown }
  ).codingProblemProgress as unknown as {
    upsert: (args: {
      where: {
        userId_problemSlug: {
          userId: bigint;
  ...
  const progress = await codingProblemProgressDelegate.upsert({
    where: {
      userId_problemSlug: {
        userId,
        problemSlug,
