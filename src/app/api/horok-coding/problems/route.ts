...
export async function POST(req: Request) {
  ...
  const codingProblemDelegate = (
    prisma as typeof prisma & { codingProblem: unknown }
  ).codingProblem as unknown as {
    findFirst: typeof prisma.codingProblem.findFirst;
    create: (args: {
  ...
  const existingProblem = await codingProblemDelegate.findFirst({
    where: {
      OR: [{ number }, { slug }],
    },
    select: {
  ...
  const problem = await codingProblemDelegate.create({
    data: {
      number,
      slug,
      title,
